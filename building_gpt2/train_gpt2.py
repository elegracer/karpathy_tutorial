...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # not really a 'bias', more of a mask, but following the OpenAI/HF naming thgough
...
class GPTConfig:
    ...
    # numebr of tokens: 50,000 BPE merges + 256 bytes tokens + 1 <|endoftext|> token
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        assert len(sd_keys_hf) == len(sd_keys), \
            f"mismatched_keys: {len(sd_keys_hf)} != {len(sd_keys)}"
        for k in sd_keys_hf:
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        # start with all of the candidate parameters (that requrie grad)
        ...
        # Crate AdamW optimizer and use the fused version if it is available
